...
public class UserCreatedConsumer {
    ...
    @Observed(name = "consume-user-created-event-1")
    @KafkaListener(
    ...
    public void consume(
        ...
        // ===== 1. EXTRACT the W3C traceparent from Kafka headers =====
        // This restores the trace context the producer injected
        // (which itself was restored from OutboxEvent.traceparent in MySQL).
        Context extractedContext = openTelemetry
                .getPropagators()
                .getTextMapPropagator()
                .extract(Context.root(), record, kafkaGetter);

        // ===== 2. Start a CHILD span under the extracted context =====
        Span span = tracer.spanBuilder("notification.user-created.consume")
        ...
        try (Scope scope = span.makeCurrent()) {
            ...
            log.info("Processing Event => EventId={}", eventId);
            ...
            log.error(
                    "Consumer Processing Failed => EventId={}",
